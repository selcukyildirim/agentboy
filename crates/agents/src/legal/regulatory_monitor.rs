...
impl Agent for RegulatoryMonitorAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "legal.regulatory-monitor".to_string(), version: "2.0.0".to_string(), name: "Regulatory Monitor".to_string(), department: "Legal".to_string(), description: "Monitor regulatory changes, assess impact on business, and track compliance deadlines".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "spreadsheet.analyze".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 }, rag_enabled: false, output_schema: None, max_cost_usd: None,
            input_schema: vec![
                InputField::new("regulations", "Regulations", InputKind::File, true),
            ],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let csv = input["regulations"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'regulations' CSV".to_string()))?;
        let regs = csv_util::parse_csv_to_maps(csv)?;
        if regs.is_empty() {
            return Err(AppError::Validation("No regulations".to_string()));
        }
        ...
        analysis.sort_by(|a, b| {
            a["days_to_comply"]
                ...
                .partial_cmp(&b["days_to_comply"].as_f64().unwrap_or(f64::INFINITY))
        ...
        let llm = ctx
            .call_llm(
                "Monitor regulations.",
                &format!(
                    "Regulations ({} total, {} critical, {} non-compliant):\n{}\n\nActions?",
                    regs.len(),
                    critical,
                    non_compliant,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_regulations": regs.len(), "critical": critical, "non_compliant": non_compliant }, "regulations": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for RegulatoryMonitorAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("GDPR update is critical."))
    }
    ...
    async fn test_regulations() {
        ...
        let r = RegulatoryMonitorAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(RegulatoryMonitorAgent::new()
            .execute_with_context(serde_json::json!({"regulations":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            RegulatoryMonitorAgent::new().manifest().id,
