...
impl Agent for DeadlineTrackerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "legal.deadline-tracker".to_string(),
            version: "2.0.0".to_string(),
            name: "Deadline Tracker".to_string(),
            department: "Legal".to_string(),
            description:
                "Track legal deadlines, filing dates, statute of limitations, and renewal dates"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["deadlines"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'deadlines' CSV".to_string()))?;
        let deadlines = csv_util::parse_csv_to_maps(csv)?;
        if deadlines.is_empty() {
            return Err(AppError::Validation("No deadlines".to_string()));
        }
        ...
        analysis.sort_by(|a, b| {
            a["days_remaining"]
                ...
                .partial_cmp(&b["days_remaining"].as_f64().unwrap_or(f64::INFINITY))
        ...
        let llm = ctx
            .call_llm(
                ...
                &format!(
                    "Deadlines ({} total, {} overdue, {} urgent):\n{}\n\nPriorities?",
                    deadlines.len(),
                    overdue,
                    urgent,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_deadlines": deadlines.len(), "overdue": overdue, "urgent": urgent }, "deadlines": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for DeadlineTrackerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Tax filing is overdue."))
    }
    ...
    async fn test_deadlines() {
        ...
        let r = DeadlineTrackerAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(DeadlineTrackerAgent::new()
            .execute_with_context(serde_json::json!({"deadlines":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            DeadlineTrackerAgent::new().manifest().id,
