...
impl Agent for DeadlineTrackerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "legal.deadline-tracker".to_string(), version: "2.0.0".to_string(), name: "Deadline Tracker".to_string(), department: "Legal".to_string(), description: "Track legal deadlines, filing dates, statute of limitations, and renewal dates".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "spreadsheet.analyze".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 }, rag_enabled: false, output_schema: None, max_cost_usd: None,
            input_schema: vec![
                InputField::new("deadlines", "Deadlines", InputKind::File, true),
            ],
        }
    }
