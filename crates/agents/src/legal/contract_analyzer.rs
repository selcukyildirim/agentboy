...
impl Agent for ContractAnalyzerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "legal.contract-analyzer".to_string(), version: "2.0.0".to_string(), name: "Contract Analyzer".to_string(), department: "Legal".to_string(), description: "Analyze contracts for key terms, obligations, renewal dates, and risk clauses".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "document.analyze".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 }, rag_enabled: false, output_schema: None, max_cost_usd: None,
            input_schema: vec![
                InputField::new("contracts", "Contracts", InputKind::File, true).with_example("contract_id,party,value,expiry\nC1,Acme,50000,2025-01-01"),
            ],
        }
    }
