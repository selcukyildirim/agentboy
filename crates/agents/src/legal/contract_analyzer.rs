...
impl Agent for ContractAnalyzerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "legal.contract-analyzer".to_string(),
            version: "2.0.0".to_string(),
            name: "Contract Analyzer".to_string(),
            department: "Legal".to_string(),
            description:
                "Analyze contracts for key terms, obligations, renewal dates, and risk clauses"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
            ...
            max_cost_usd: None,
            input_schema: vec![
                InputField::new("contracts", "Contracts", InputKind::File, true)
                    .with_example("contract_id,party,value,expiry\nC1,Acme,50000,2025-01-01"),
            ],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let csv = input["contracts"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'contracts' CSV".to_string()))?;
        let contracts = csv_util::parse_csv_to_maps(csv)?;
        if contracts.is_empty() {
            return Err(AppError::Validation("No contracts".to_string()));
        }
        ...
        let total_value: f64 = analysis
            .iter()
            .map(|a| a["annual_value"].as_f64().unwrap_or(0.0))
            .sum();
        ...
        let llm = ctx
            .call_llm(
                "Analyze contracts.",
                &format!(
                    "Contracts ({} total, value: {:.0}, {} high risk):\n{}\n\nKey risks?",
                    contracts.len(),
                    total_value,
                    high_risk,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_contracts": contracts.len(), "total_value": total_value, "high_risk": high_risk }, "contracts": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for ContractAnalyzerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Contract C is high risk."))
    }
    ...
    async fn test_contracts() {
        ...
        let r = ContractAnalyzerAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(ContractAnalyzerAgent::new()
            .execute_with_context(serde_json::json!({"contracts":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            ContractAnalyzerAgent::new().manifest().id,
