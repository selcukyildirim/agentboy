...
impl Agent for ComplianceCheckAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "legal.compliance-check".to_string(),
            version: "2.0.0".to_string(),
            name: "Compliance Check".to_string(),
            department: "Legal".to_string(),
            description: "Verify regulatory compliance across policies, procedures, and filings"
                .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["requirements"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'requirements' CSV".to_string()))?;
        ...
        let critical = analysis
            .iter()
            .filter(|a| a["status"] == "critical_gap")
            .count();
        ...
        Ok(
            serde_json::json!({ "summary": { "total": reqs.len(), "compliant": compliant, "gaps": gaps, "critical": critical }, "requirements": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for ComplianceCheckAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    async fn test_compliance() {
        ...
        let r = ComplianceCheckAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(ComplianceCheckAgent::new()
            .execute_with_context(serde_json::json!({"requirements":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            ComplianceCheckAgent::new().manifest().id,
