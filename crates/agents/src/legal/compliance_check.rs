...
impl Agent for ComplianceCheckAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "legal.compliance-check".to_string(), version: "2.0.0".to_string(), name: "Compliance Check".to_string(), department: "Legal".to_string(), description: "Verify regulatory compliance across policies, procedures, and filings".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "document.validate".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 }, rag_enabled: false, output_schema: None, max_cost_usd: None,
            input_schema: vec![
                InputField::new("requirements", "Compliance Requirements", InputKind::File, true),
            ],
        }
    }
