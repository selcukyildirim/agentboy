...
impl Agent for FleetManagerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "logistics.fleet-manager".to_string(),
            version: "2.0.0".to_string(),
            name: "Fleet Manager".to_string(),
            department: "Logistics".to_string(),
            description: "Monitor fleet utilization, vehicle health, and maintenance schedules"
                .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["vehicles"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'vehicles' CSV".to_string()))?;
        ...
        let total_fuel: f64 = analysis
            .iter()
            .map(|a| a["fuel_cost"].as_f64().unwrap_or(0.0))
            .sum();
        let needs_service = analysis
            .iter()
            .filter(|a| a["status"] == "needs_service")
            .count();
        let underutilized = analysis
            .iter()
            .filter(|a| a["status"] == "underutilized")
            .count();
        ...
        Ok(
            serde_json::json!({ "summary": { "total_vehicles": vehicles.len(), "total_fuel_cost": total_fuel, "needs_service": needs_service, "underutilized": underutilized }, "vehicles": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for FleetManagerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(FleetManagerAgent::new()
            .execute_with_context(serde_json::json!({"vehicles":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            FleetManagerAgent::new().manifest().id,
