...
impl Agent for WarehouseOptimizerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "logistics.warehouse-optimizer".to_string(),
            version: "2.0.0".to_string(),
            name: "Warehouse Optimizer".to_string(),
            department: "Logistics".to_string(),
            description: "Optimize warehouse layout, storage utilization, and picking efficiency"
                .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["zones"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'zones' CSV".to_string()))?;
        ...
        let overflow = analysis
            .iter()
            .filter(|a| a["status"] == "overflow")
            .count();
        let total_picks: f64 = analysis
            .iter()
            .map(|a| a["daily_picks"].as_f64().unwrap_or(0.0))
            .sum();
        ...
        let llm = ctx
            .call_llm(
                ...
                &format!(
                    "Warehouse ({} zones, {} overflow, {} daily picks):\n{}\n\nOptimize?",
                    zones.len(),
                    overflow,
                    total_picks,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_zones": zones.len(), "overflow": overflow, "total_daily_picks": total_picks }, "zones": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for WarehouseOptimizerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Zone A is overflowing."))
    }

    #[tokio::test]
    async fn test_warehouse() {
        let input = serde_json::json!({ "zones": "zone,capacity_units,used_units,daily_picks\nA,1000,980,50\nB,1000,500,30" });
        let r = WarehouseOptimizerAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(WarehouseOptimizerAgent::new()
            .execute_with_context(serde_json::json!({"zones":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            WarehouseOptimizerAgent::new().manifest().id,
