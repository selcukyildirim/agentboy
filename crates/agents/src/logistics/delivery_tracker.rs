...
impl Agent for DeliveryTrackerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "logistics.delivery-tracker".to_string(),
            version: "2.0.0".to_string(),
            name: "Delivery Tracker".to_string(),
            department: "Logistics".to_string(),
            description: "Track delivery performance, SLA compliance, and identify delays"
                .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let on_time = deliveries
            .iter()
            .filter(|d| {
                csv_util::record_get_str(d, "status") == "delivered"
                    && csv_util::record_get_f64(d, "delay_hours") <= 0.0
            })
            .count();
        let late = deliveries
            .iter()
            .filter(|d| csv_util::record_get_f64(d, "delay_hours") > 0.0)
            .count();
        let delays: Vec<f64> = deliveries
            .iter()
...
impl Default for DeliveryTrackerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("SLA is below target."))
    }

    #[tokio::test]
    async fn test_deliveries() {
        let input = serde_json::json!({ "deliveries": "delivery_id,status,delay_hours\nD1,delivered,0\nD2,delivered,-1\nD3,delivered,3" });
        let r = DeliveryTrackerAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(DeliveryTrackerAgent::new()
    ...
    fn test_manifest() {
        assert_eq!(
            DeliveryTrackerAgent::new().manifest().id,
