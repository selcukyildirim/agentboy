...
impl Agent for HiringPipelineAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            ...
            description: "Analyze hiring pipeline efficiency, time-to-hire, and conversion rates by stage".to_string(),
            tier: AgentTier::Free,
            ...
            max_cost_usd: None,
            input_schema: vec![
                InputField::new("candidates", "Candidates", InputKind::File, true),
            ],
        }
    }
    ...
    async fn execute_with_context(&self, input: serde_json::Value, ctx: &dyn AgentContext) -> AppResult<serde_json::Value> {
        ...
        let stages = vec!["applied", "screening", "interview", "offer", "hired", "rejected"];
        ...
        let conversion = |from: u64, to: u64| if from > 0 { to as f64 / from as f64 * 100.0 } else { 0.0 };

        let time_to_hires: Vec<f64> = candidates.iter()
            .filter(|c| csv_util::record_get_str(c, "stage") == "hired")
            .filter_map(|c| c.get("days_to_hire").and_then(|v| v.parse::<f64>().ok()))
            .collect();
        let avg_time_to_hire = if !time_to_hires.is_empty() { time_to_hires.iter().sum::<f64>() / time_to_hires.len() as f64 } else { 0.0 };
        ...
        let stage_summary: Vec<serde_json::Value> = stages.iter().filter_map(|s| {
...
impl Default for HiringPipelineAgent { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn make_ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Interview-to-offer conversion is low. Improve screening."))
    }
    ...
    async fn test_pipeline() {
        ...
        let result = HiringPipelineAgent::new().execute_with_context(input, &ctx).await.unwrap();
    ...
    async fn test_empty() {
        let ctx = make_ctx();
        assert!(HiringPipelineAgent::new().execute_with_context(serde_json::json!({ "candidates": "" }), &ctx).await.is_err());
    }
