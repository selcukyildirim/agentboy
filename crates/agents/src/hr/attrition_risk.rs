...
impl Agent for AttritionRiskAgent {
    ...
    async fn execute_with_context(
        ...
        let mut scored: Vec<serde_json::Value> = employees
            .iter()
            .map(|e| {
                ...
                let risk =
                    calculate_attrition_risk(satisfaction, tenure, performance, salary_ratio);
                let tier = if risk > 70 {
                    "high"
                } else if risk > 40 {
                    "medium"
        ...
        scored.sort_by(|a, b| {
            b["risk_score"]
                .as_u64()
                .unwrap_or(0)
                .partial_cmp(&a["risk_score"].as_u64().unwrap_or(0))
...
impl Default for AttritionRiskAgent {
    fn default() -> Self {
        Self::new()
    }
}

fn calculate_attrition_risk(
    satisfaction: f64,
    tenure: f64,
    performance: f64,
    salary_ratio: f64,
...
mod tests {
    ...
    fn make_ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response(
            "Alice is high risk. Recommend retention discussion.",
        ))
    }
    ...
    async fn test_attrition() {
        ...
        let result = AttritionRiskAgent::new()
