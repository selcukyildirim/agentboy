...
impl Agent for TrainingROIAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            ...
            description: "Measure training program effectiveness and return on investment".to_string(),
            tier: AgentTier::Free,
            ...
            max_cost_usd: None,
            input_schema: vec![
                InputField::new("programs", "Training Programs", InputKind::File, true),
            ],
        }
    }
    ...
    async fn execute_with_context(&self, input: serde_json::Value, ctx: &dyn AgentContext) -> AppResult<serde_json::Value> {
        ...
        if programs.is_empty() {
            return Err(AppError::Validation("No training programs found".to_string()));
        }

        let mut program_analysis: Vec<serde_json::Value> = programs.iter().map(|p| {
            ...
            let cost_per_participant = if participants > 0.0 { cost / participants } else { 0.0 };
            let roi = if cost > 0.0 { (productivity_gain * cost / 100.0) / cost } else { 0.0 };

            let effectiveness = if productivity_gain > 20.0 && completion_rate > 0.8 { "high" }
                else if productivity_gain > 10.0 { "medium" }
                else { "low" };
        ...
        let total_cost: f64 = program_analysis.iter().map(|p| p["cost"].as_f64().unwrap_or(0.0)).sum();
        let total_participants: u64 = program_analysis.iter().map(|p| p["participants"].as_u64().unwrap_or(0)).sum();
        let high_effectiveness = program_analysis.iter().filter(|p| p["effectiveness"] == "high").count();

        program_analysis.sort_by(|a, b| b["productivity_gain_pct"].as_f64().unwrap_or(0.0).partial_cmp(&a["productivity_gain_pct"].as_f64().unwrap_or(0.0)).unwrap_or(std::cmp::Ordering::Equal));
...
impl Default for TrainingROIAgent { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn make_ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Leadership program has highest ROI. Expand it."))
    }
    ...
    async fn test_training_roi() {
        ...
        let result = TrainingROIAgent::new().execute_with_context(input, &ctx).await.unwrap();
    ...
    async fn test_empty() {
        let ctx = make_ctx();
        assert!(TrainingROIAgent::new().execute_with_context(serde_json::json!({ "programs": "" }), &ctx).await.is_err());
    }
