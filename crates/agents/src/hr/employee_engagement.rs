...
impl Agent for EmployeeEngagementAgent {
    ...
    async fn execute_with_context(&self, input: serde_json::Value, ctx: &dyn AgentContext) -> AppResult<serde_json::Value> {
        ...
        if surveys.is_empty() {
            return Err(AppError::Validation("No survey responses found".to_string()));
        }

        let by_dept = csv_util::group_by(&surveys, "department");

        let mut dept_scores: Vec<serde_json::Value> = by_dept.iter().map(|(dept, responses)| {
            let overall: Vec<f64> = responses.iter().filter_map(|r| r.get("overall_score").and_then(|v| v.parse::<f64>().ok())).collect();
            let worklife: Vec<f64> = responses.iter().filter_map(|r| r.get("work_life_balance").and_then(|v| v.parse::<f64>().ok())).collect();
            let growth: Vec<f64> = responses.iter().filter_map(|r| r.get("growth_opportunity").and_then(|v| v.parse::<f64>().ok())).collect();
            let management: Vec<f64> = responses.iter().filter_map(|r| r.get("management_score").and_then(|v| v.parse::<f64>().ok())).collect();

            let avg = |vals: &[f64]| if !vals.is_empty() { vals.iter().sum::<f64>() / vals.len() as f64 } else { 0.0 };

            let overall_avg = avg(&overall);
            let engagement_level = if overall_avg >= 4.0 { "high" } else if overall_avg >= 3.0 { "moderate" } else { "low" };
        ...
        dept_scores.sort_by(|a, b| a["overall_score"].as_f64().unwrap_or(0.0).partial_cmp(&b["overall_score"].as_f64().unwrap_or(0.0)).unwrap_or(std::cmp::Ordering::Equal));

        let all_overall: Vec<f64> = surveys.iter().filter_map(|r| r.get("overall_score").and_then(|v| v.parse::<f64>().ok())).collect();
        let company_avg = if !all_overall.is_empty() { all_overall.iter().sum::<f64>() / all_overall.len() as f64 } else { 0.0 };
...
impl Default for EmployeeEngagementAgent { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn make_ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Engineering has lowest engagement. Focus on work-life balance."))
    }
    ...
    async fn test_engagement() {
        ...
        let result = EmployeeEngagementAgent::new().execute_with_context(input, &ctx).await.unwrap();
    ...
    async fn test_empty() {
        let ctx = make_ctx();
        assert!(EmployeeEngagementAgent::new().execute_with_context(serde_json::json!({ "surveys": "" }), &ctx).await.is_err());
    }
