...
impl Agent for CapacityPlannerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "operations.capacity-planner".to_string(),
            version: "2.0.0".to_string(),
            name: "Capacity Planner".to_string(),
            department: "Operations".to_string(),
            description: "Analyze production capacity, utilization rates, and identify bottlenecks"
                .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["machines"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'machines' CSV".to_string()))?;
        let machines = csv_util::parse_csv_to_maps(csv)?;
        if machines.is_empty() {
            return Err(AppError::Validation("No machines".to_string()));
        }
        ...
        analysis.sort_by(|a, b| {
            b["utilization_pct"]
                .as_f64()
                .unwrap_or(0.0)
                .partial_cmp(&a["utilization_pct"].as_f64().unwrap_or(0.0))
        ...
        let bottlenecks = analysis
            .iter()
            .filter(|a| a["status"] == "bottleneck")
            .count();

        let llm = ctx
            .call_llm(
                "Analyze capacity and recommend balancing.",
                &format!(
                    "Capacity ({} machines, avg util: {:.1}%):\n{}\n\nRecommendations?",
                    machines.len(),
                    avg_util,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_machines": machines.len(), "avg_utilization": avg_util, "bottlenecks": bottlenecks }, "machines": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for CapacityPlannerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Machine A is a bottleneck."))
    }

    #[tokio::test]
    async fn test_capacity() {
        let input = serde_json::json!({ "machines": "machine,line,capacity_hours,used_hours\nM1,Line A,100,95\nM2,Line A,100,40" });
        let r = CapacityPlannerAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(CapacityPlannerAgent::new()
            .execute_with_context(serde_json::json!({"machines":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            CapacityPlannerAgent::new().manifest().id,
