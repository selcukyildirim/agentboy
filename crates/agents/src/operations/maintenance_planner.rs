...
impl Agent for MaintenancePlannerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "operations.maintenance-planner".to_string(),
            version: "2.0.0".to_string(),
            name: "Maintenance Planner".to_string(),
            department: "Operations".to_string(),
            description: "Schedule preventive maintenance, predict failures, and minimize downtime"
                .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["equipment"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'equipment' CSV".to_string()))?;
        let equipment = csv_util::parse_csv_to_maps(csv)?;
        if equipment.is_empty() {
            return Err(AppError::Validation("No equipment".to_string()));
        }
        ...
        analysis.sort_by(|a, b| {
            a["urgency"]
                .as_str()
                .cmp(&b["urgency"].as_str())
                .then_with(|| {
                    b["hours_since_maintenance"]
                        .as_f64()
                        .unwrap_or(0.0)
                        .partial_cmp(&a["hours_since_maintenance"].as_f64().unwrap_or(0.0))
                        .unwrap_or(std::cmp::Ordering::Equal)
                })
        });

        let overdue = analysis
            .iter()
            .filter(|a| a["urgency"] == "overdue")
            .count();
        let due_soon = analysis
            .iter()
        ...
        let llm = ctx
            .call_llm(
                ...
                &format!(
                    "Maintenance ({} equipment, {} overdue, {} due soon):\n{}\n\nSchedule?",
                    equipment.len(),
                    overdue,
                    due_soon,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_equipment": equipment.len(), "overdue": overdue, "due_soon": due_soon }, "equipment": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for MaintenancePlannerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response(
            "Overhaul Machine M1 immediately.",
        ))
    }
    ...
    async fn test_maintenance() {
        ...
        let r = MaintenancePlannerAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(MaintenancePlannerAgent::new()
            .execute_with_context(serde_json::json!({"equipment":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            MaintenancePlannerAgent::new().manifest().id,
