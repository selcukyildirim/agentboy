...
impl Agent for InventoryOptimizerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "operations.inventory-optimizer".to_string(),
            version: "2.0.0".to_string(),
            name: "Inventory Optimizer".to_string(),
            department: "Operations".to_string(),
            description:
                "Optimize inventory levels, identify dead stock, and recommend reorder points"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
            ...
            max_cost_usd: None,
            input_schema: vec![
                InputField::new("inventory", "Inventory", InputKind::File, true)
                    .with_example("sku,on_hand,avg_daily_demand,lead_time_days\nSKU1,50,10,7"),
            ],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let csv = input["inventory"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'inventory' CSV".to_string()))?;
        let items = csv_util::parse_csv_to_maps(csv)?;
        if items.is_empty() {
            return Err(AppError::Validation("No inventory items".to_string()));
        }

        let mut analysis: Vec<serde_json::Value> = items
            .iter()
            .map(|i| {
        ...
        analysis.sort_by(|a, b| {
            a["days_of_stock"]
                ...
                .partial_cmp(&b["days_of_stock"].as_f64().unwrap_or(f64::INFINITY))
                .unwrap_or(std::cmp::Ordering::Equal)
        });

        let total_value: f64 = analysis
            .iter()
            .map(|a| a["inventory_value"].as_f64().unwrap_or(0.0))
            .sum();
        let reorder_count = analysis.iter().filter(|a| a["status"] == "reorder").count();
        let overstock_count = analysis
            .iter()
            .filter(|a| a["status"] == "overstock")
            .count();
        ...
        let llm = ctx
            .call_llm(
                "Analyze inventory and recommend optimizations.",
                &user_prompt,
            )
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_items": items.len(), "total_value": total_value, "reorder": reorder_count, "overstock": overstock_count }, "items": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for InventoryOptimizerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Reorder SKU-A immediately."))
    }
    ...
    async fn test_inventory() {
        ...
        let r = InventoryOptimizerAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(InventoryOptimizerAgent::new()
            .execute_with_context(serde_json::json!({"inventory":""}), &ctx())
            .await
            .is_err());
    }
    #[tokio::test]
    ...
    fn test_manifest() {
        assert_eq!(
            InventoryOptimizerAgent::new().manifest().id,
            "operations.inventory-optimizer"
        );
    }
}
