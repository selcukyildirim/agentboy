...
impl Agent for SupplyChainRiskAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "operations.supply-chain-risk".to_string(), version: "2.0.0".to_string(), name: "Supply Chain Risk".to_string(), department: "Operations".to_string(), description: "Identify supply chain vulnerabilities, single-source risks, and recommend mitigation".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "spreadsheet.analyze".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 }, rag_enabled: false, output_schema: None, max_cost_usd: None,
            input_schema: vec![
                InputField::new("suppliers", "Suppliers (JSON)", InputKind::File, true).with_example("supplier,unit_price,lead_time_days,quality_score\nAcme,25,7,95"),
            ],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let csv = input["suppliers"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'suppliers' CSV".to_string()))?;
        let suppliers = csv_util::parse_csv_to_maps(csv)?;
        if suppliers.is_empty() {
            return Err(AppError::Validation("No suppliers".to_string()));
        }
        ...
        let llm = ctx
            .call_llm(
                "Analyze supply chain risks and recommend mitigations.",
                &format!(
                    "Supply Chain ({} components, {} high risk):\n{}\n\nMitigations?",
                    risks.len(),
                    high_risk,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_components": risks.len(), "high_risk": high_risk }, "risks": risks, "llm_analysis": llm }),
        )
    }
}
impl Default for SupplyChainRiskAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    async fn test_risk() {
        ...
        let r = SupplyChainRiskAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(SupplyChainRiskAgent::new()
            .execute_with_context(serde_json::json!({"suppliers":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            SupplyChainRiskAgent::new().manifest().id,
