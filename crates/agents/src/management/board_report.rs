...
impl Agent for BoardReportAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "management.board-report".to_string(),
            version: "2.0.0".to_string(),
            name: "Board Report".to_string(),
            department: "Management".to_string(),
            description:
                "Generate board-ready executive summary with key metrics and strategic updates"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
            ...
            output_schema: None,
            max_cost_usd: None,
            input_schema: vec![InputField::new("metrics", "Metrics", InputKind::File, true)
                .with_example("metric,value,period\nRevenue,100000,Q1")],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let csv = input["metrics"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'metrics' CSV".to_string()))?;
        ...
        Ok(
            serde_json::json!({ "summary": { "total_metrics": metrics.len(), "improving": improving, "declining": declining }, "metrics": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for BoardReportAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response(
            "Revenue is growing. Costs need attention.",
        ))
    }
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(BoardReportAgent::new()
            .execute_with_context(serde_json::json!({"metrics":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            BoardReportAgent::new().manifest().id,
