...
impl Agent for OKRAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "management.okr".to_string(), version: "2.0.0".to_string(), name: "OKR".to_string(), department: "Management".to_string(), description: "Track OKR progress across teams, identify at-risk key results, and align priorities".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "spreadsheet.analyze".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 }, rag_enabled: false, output_schema: None, max_cost_usd: None,
            input_schema: vec![
                InputField::new("okrs", "OKRs", InputKind::File, true).with_example("objective,key_result,progress\nGrow,Sign 10 clients,0.6"),
            ],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let csv = input["okrs"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'okrs' CSV".to_string()))?;
        let okrs = csv_util::parse_csv_to_maps(csv)?;
        if okrs.is_empty() {
            return Err(AppError::Validation("No OKRs".to_string()));
        }
        ...
        let llm = ctx
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_okrs": okrs.len(), "on_track": on_track, "behind": behind }, "okrs": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for OKRAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Focus on the behind OKR."))
    }
    ...
    async fn test_empty() {
        assert!(OKRAgent::new()
            .execute_with_context(serde_json::json!({"okrs":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(OKRAgent::new().manifest().id, "management.okr");
    }
}
