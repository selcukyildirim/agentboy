...
impl Agent for StrategicInitiativeAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "management.strategic-initiative".to_string(),
            version: "2.0.0".to_string(),
            name: "Strategic Initiative".to_string(),
            department: "Management".to_string(),
            description:
                "Track strategic initiative progress, milestones, and alignment with goals"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["initiatives"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'initiatives' CSV".to_string()))?;
        let inits = csv_util::parse_csv_to_maps(csv)?;
        if inits.is_empty() {
            return Err(AppError::Validation("No initiatives".to_string()));
        }
        ...
        analysis.sort_by(|a, b| {
            a["progress_pct"]
                .as_f64()
                .unwrap_or(0.0)
                .partial_cmp(&b["progress_pct"].as_f64().unwrap_or(0.0))
                .unwrap_or(std::cmp::Ordering::Equal)
        });

        let behind = analysis
            .iter()
            .filter(|a| a["progress_status"] == "behind")
            .count();
        let over_budget = analysis
            .iter()
            .filter(|a| a["budget_status"] == "over_budget")
            .count();
        ...
        let llm = ctx
            .call_llm(
                ...
                &format!(
                    "Initiatives ({} total, {} behind, {} over budget):\n{}\n\nActions?",
                    inits.len(),
                    behind,
                    over_budget,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total": inits.len(), "behind": behind, "over_budget": over_budget }, "initiatives": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for StrategicInitiativeAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response(
            "Digital transformation is behind.",
        ))
    }
    ...
    async fn test_initiatives() {
        ...
        let r = StrategicInitiativeAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(StrategicInitiativeAgent::new()
            .execute_with_context(serde_json::json!({"initiatives":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            StrategicInitiativeAgent::new().manifest().id,
