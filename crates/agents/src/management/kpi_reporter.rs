...
impl Agent for KPIReporterAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "management.kpi-reporter".to_string(),
            version: "2.0.0".to_string(),
            name: "KPI Reporter".to_string(),
            department: "Management".to_string(),
            description:
                "Track KPIs across departments, identify trends, and highlight performance gaps"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
            ...
            output_schema: None,
            max_cost_usd: None,
            input_schema: vec![InputField::new("kpis", "KPIs", InputKind::File, true)
                .with_example("kpi,target,actual,unit\nRevenue,100000,95000,USD")],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let csv = input["kpis"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'kpis' CSV".to_string()))?;
        let kpis = csv_util::parse_csv_to_maps(csv)?;
        if kpis.is_empty() {
            return Err(AppError::Validation("No KPIs".to_string()));
        }
        ...
        analysis.sort_by(|a, b| {
            a["variance_pct"]
                .as_f64()
                .unwrap_or(0.0)
                .partial_cmp(&b["variance_pct"].as_f64().unwrap_or(0.0))
                .unwrap_or(std::cmp::Ordering::Equal)
        });

        let off_track = analysis
            .iter()
            .filter(|a| a["status"] == "off_track")
            .count();
        let on_track = analysis
            .iter()
        ...
        let llm = ctx
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_kpis": kpis.len(), "on_track": on_track, "off_track": off_track }, "kpis": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for KPIReporterAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Revenue KPI is off track."))
    }

    #[tokio::test]
    async fn test_kpis() {
        let input = serde_json::json!({ "kpis": "name,department,current_value,target_value\nRevenue,Finance,800,1000\nNPS,Sales,45,40\nDefects,Ops,2,5" });
        let r = KPIReporterAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(KPIReporterAgent::new()
            .execute_with_context(serde_json::json!({"kpis":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            KPIReporterAgent::new().manifest().id,
            "management.kpi-reporter"
        );
    }
}
