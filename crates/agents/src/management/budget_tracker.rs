...
impl Agent for BudgetTrackerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "management.budget-tracker".to_string(), version: "2.0.0".to_string(), name: "Budget Tracker".to_string(), department: "Management".to_string(), description: "Track budget execution across departments and forecast year-end position".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "spreadsheet.analyze".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 } }
    }
