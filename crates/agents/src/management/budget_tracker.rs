...
impl Agent for BudgetTrackerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "management.budget-tracker".to_string(),
            version: "2.0.0".to_string(),
            name: "Budget Tracker".to_string(),
            department: "Management".to_string(),
            description: "Track budget execution across departments and forecast year-end position"
                .to_string(),
            tier: AgentTier::Free,
            skills: vec![
            ...
            output_schema: None,
            max_cost_usd: None,
            input_schema: vec![InputField::new("budgets", "Budgets", InputKind::File, true)
                .with_example("department,budget,spent\nSales,100000,40000")],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let csv = input["budgets"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'budgets' CSV".to_string()))?;
        ...
        let total_allocated: f64 = analysis
            .iter()
            .map(|a| a["allocated"].as_f64().unwrap_or(0.0))
            .sum();
        let total_spent: f64 = analysis
            .iter()
            .map(|a| a["spent"].as_f64().unwrap_or(0.0))
            .sum();
        ...
        let llm = ctx
            .call_llm(
                ...
                &format!(
                    "Budget ({} departments, total: {:.0}, spent: {:.0}, {} over):\n{}\n\nActions?",
                    budgets.len(),
                    total_allocated,
                    total_spent,
                    over,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_departments": budgets.len(), "total_allocated": total_allocated, "total_spent": total_spent, "over_budget": over }, "departments": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for BudgetTrackerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Marketing is over budget."))
    }

    #[tokio::test]
    async fn test_budget() {
        let input = serde_json::json!({ "budgets": "department,allocated,spent\nEng,100000,85000\nMkt,50000,55000" });
        let r = BudgetTrackerAgent::new()
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(BudgetTrackerAgent::new()
            .execute_with_context(serde_json::json!({"budgets":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            BudgetTrackerAgent::new().manifest().id,
            "management.budget-tracker"
        );
    }
}
