...
impl Agent for ChangeRequestAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "pmo.change-request".to_string(),
            version: "2.0.0".to_string(),
            name: "Change Request".to_string(),
            department: "PMO".to_string(),
            description:
                "Analyze change requests, assess impact on schedule/budget, and recommend approvals"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["changes"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'changes' CSV".to_string()))?;
        ...
        let llm = ctx
            .call_llm(
                ...
                &format!(
                    "Change Requests ({} total, {} approve, {} reject):\n{}\n\nDecisions?",
                    changes.len(),
                    approve,
                    reject,
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_changes": changes.len(), "approve": approve, "reject": reject }, "changes": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for ChangeRequestAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("CR-2 should be rejected."))
    }
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(ChangeRequestAgent::new()
            .execute_with_context(serde_json::json!({"changes":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            ChangeRequestAgent::new().manifest().id,
            "pmo.change-request"
        );
    }
}
