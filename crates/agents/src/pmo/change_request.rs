...
impl Agent for ChangeRequestAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "pmo.change-request".to_string(), version: "2.0.0".to_string(), name: "Change Request".to_string(), department: "PMO".to_string(), description: "Analyze change requests, assess impact on schedule/budget, and recommend approvals".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "spreadsheet.analyze".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 } }
    }
