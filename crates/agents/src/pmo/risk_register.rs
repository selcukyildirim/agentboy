...
impl RiskRegisterAgent { pub fn new() -> Self { Self } }
...
impl Agent for RiskRegisterAgent {
    ...
    async fn execute_with_context(&self, input: serde_json::Value, ctx: &dyn AgentContext) -> AppResult<serde_json::Value> {
        ...
        analysis.sort_by(|a, b| b["score"].as_f64().unwrap_or(0.0).partial_cmp(&a["score"].as_f64().unwrap_or(0.0)).unwrap_or(std::cmp::Ordering::Equal));

        let high = analysis.iter().filter(|a| a["level"] == "high").count();
        let unmitigated = analysis.iter().filter(|a| a["mitigation_status"] == "unmitigated").count();

        let llm = ctx.call_llm("Analyze risk register.", &format!("Risks ({} total, {} high, {} unmitigated):\n{}\n\nPriorities?", risks.len(), high, unmitigated, serde_json::to_string_pretty(&analysis).unwrap_or_default())).await?;
        Ok(serde_json::json!({ "summary": { "total_risks": risks.len(), "high": high, "unmitigated": unmitigated }, "risks": analysis, "llm_analysis": llm }))
    }
}
impl Default for RiskRegisterAgent { fn default() -> Self { Self::new() } }

#[cfg(test)]
mod tests {
    use super::*;
    use agent_runtime::{MockAgentContext, MockLlmProvider};
    fn ctx() -> MockAgentContext { MockAgentContext::new(MockLlmProvider::with_response("Risk R1 is unmitigated and high.")) }
    ...
    async fn test_risks() {
        ...
        let r = RiskRegisterAgent::new().execute_with_context(input, &ctx()).await.unwrap();
        assert_eq!(r["summary"]["high"], 1);
        assert_eq!(r["summary"]["unmitigated"], 1);
    }
    #[tokio::test]
    async fn test_empty() { assert!(RiskRegisterAgent::new().execute_with_context(serde_json::json!({"risks":""}), &ctx()).await.is_err()); }
    #[test] fn test_manifest() { assert_eq!(RiskRegisterAgent::new().manifest().id, "pmo.risk-register"); }
}