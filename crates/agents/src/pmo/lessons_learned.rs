...
impl Agent for LessonsLearnedAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "pmo.lessons-learned".to_string(),
            version: "2.0.0".to_string(),
            name: "Lessons Learned".to_string(),
            department: "PMO".to_string(),
            description:
                "Capture and analyze project lessons learned, identify patterns and improvements"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        let csv = input["lessons"]
            .as_str()
            .ok_or_else(|| AppError::Validation("Missing 'lessons' CSV".to_string()))?;
        let lessons = csv_util::parse_csv_to_maps(csv)?;
        if lessons.is_empty() {
            return Err(AppError::Validation("No lessons".to_string()));
        }
        ...
        let llm = ctx
            .call_llm(
                ...
                &format!(
                    "Lessons Learned ({} total, {} categories, {} projects):\n{}\n\nKey patterns?",
                    lessons.len(),
                    by_category.len(),
                    by_project.len(),
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_lessons": lessons.len(), "categories": by_category.len(), "projects": by_project.len() }, "by_category": category_summary, "llm_analysis": llm }),
        )
    }
}
impl Default for LessonsLearnedAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(LessonsLearnedAgent::new()
            .execute_with_context(serde_json::json!({"lessons":""}), &ctx())
            .await
            .is_err());
    }
    #[test]
    fn test_manifest() {
        assert_eq!(
            LessonsLearnedAgent::new().manifest().id,
            "pmo.lessons-learned"
        );
    }
}
