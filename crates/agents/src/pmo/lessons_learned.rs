...
impl Agent for LessonsLearnedAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest { id: "pmo.lessons-learned".to_string(), version: "2.0.0".to_string(), name: "Lessons Learned".to_string(), department: "PMO".to_string(), description: "Capture and analyze project lessons learned, identify patterns and improvements".to_string(), tier: AgentTier::Free, skills: vec!["spreadsheet.parse".to_string(), "spreadsheet.analyze".to_string(), "llm.analysis".to_string()], permissions: AgentPermissions { filesystem_read: true, filesystem_write: false, network_llm: true }, execution: ExecutionLimits { max_steps: 30, timeout_seconds: 120 }, rag_enabled: false, output_schema: None, max_cost_usd: None,
            input_schema: vec![
                InputField::new("lessons", "Lessons Learned", InputKind::File, true),
            ],
        }
    }
