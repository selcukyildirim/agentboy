...
impl Agent for ProjectTrackerAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            id: "pmo.project-tracker".to_string(),
            version: "2.0.0".to_string(),
            name: "Project Tracker".to_string(),
            department: "PMO".to_string(),
            description: "Track project milestones, schedule variance, and critical path items"
                .to_string(),
            tier: AgentTier::Free,
            skills: vec![
    ...
    async fn execute_with_context(
        ...
        analysis.sort_by(|a, b| {
            a["schedule_variance_days"]
                .as_f64()
                .unwrap_or(0.0)
                .partial_cmp(&b["schedule_variance_days"].as_f64().unwrap_or(0.0))
        ...
        let llm = ctx
            ...
            .await?;
        Ok(
            serde_json::json!({ "summary": { "total_projects": projects.len(), "critical": critical }, "projects": analysis, "llm_analysis": llm }),
        )
    }
}
impl Default for ProjectTrackerAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response("Project Alpha is critical."))
    }
    ...
    #[tokio::test]
    async fn test_empty() {
        assert!(ProjectTrackerAgent::new()
    ...
    fn test_manifest() {
        assert_eq!(
            ProjectTrackerAgent::new().manifest().id,
            "pmo.project-tracker"
        );
    }
}
