...
impl Agent for LeadScoringAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            ...
            description: "Score and rank leads based on engagement, fit, and conversion signals".to_string(),
            tier: AgentTier::Free,
            ...
            max_cost_usd: None,
            input_schema: vec![
                InputField::new("leads", "Leads", InputKind::File, true).with_example("lead_id,company,score\nL1,Acme,85"),
            ],
        }
    }
    ...
    async fn execute_with_context(&self, input: serde_json::Value, ctx: &dyn AgentContext) -> AppResult<serde_json::Value> {
        ...
        let mut scored: Vec<serde_json::Value> = leads.iter().map(|lead| {
            ...
            let tier = if composite >= 0.8 { "hot" } else if composite >= 0.5 { "warm" } else { "cold" };
        ...
        scored.sort_by(|a, b| b["composite_score"].as_f64().unwrap_or(0.0).partial_cmp(&a["composite_score"].as_f64().unwrap_or(0.0)).unwrap_or(std::cmp::Ordering::Equal));
...
impl Default for LeadScoringAgent { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    async fn test_scoring() {
        ...
        let result = LeadScoringAgent::new().execute_with_context(input, &ctx).await.unwrap();
    ...
    async fn test_empty_leads() {
        let ctx = make_ctx();
        assert!(LeadScoringAgent::new().execute_with_context(serde_json::json!({ "leads": "" }), &ctx).await.is_err());
    }
    ...
    async fn test_sorted() {
        ...
        let result = LeadScoringAgent::new().execute_with_context(input, &ctx).await.unwrap();
        let leads = result["leads"].as_array().unwrap();
        assert!(leads[0]["composite_score"].as_f64().unwrap() >= leads[1]["composite_score"].as_f64().unwrap());
    }
