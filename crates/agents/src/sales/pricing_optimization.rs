...
impl Agent for PricingOptimizationAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            ...
            description:
                "Analyze pricing elasticity, discount impact, and optimize pricing strategies"
                    .to_string(),
            tier: AgentTier::Free,
            skills: vec![
            ...
            max_cost_usd: None,
            input_schema: vec![
                InputField::new("products", "Products", InputKind::File, true)
                    .with_example("product,cost,price,list_price\nWidget,70,100,120"),
            ],
        }
    }
    ...
    async fn execute_with_context(
        ...
        let mut product_analysis: Vec<serde_json::Value> = products
            .iter()
            .map(|p| {
                ...
                let discount_pct = if base_price > 0.0 {
                    (1.0 - current_price / base_price) * 100.0
                } else {
                    0.0
                };
                let price_vs_competitor = if competitor_price > 0.0 {
                    (current_price / competitor_price - 1.0) * 100.0
                } else {
                    0.0
                };
                let revenue = current_price * units_sold;

                let pricing_status = if price_vs_competitor > 10.0 {
                    "premium"
                } else if price_vs_competitor < -10.0 {
                    "discounted"
                } else {
                    "competitive"
                };
        ...
        product_analysis.sort_by(|a, b| {
            b["revenue"]
                .as_f64()
                .unwrap_or(0.0)
                .partial_cmp(&a["revenue"].as_f64().unwrap_or(0.0))
                .unwrap_or(std::cmp::Ordering::Equal)
        });

        let total_revenue: f64 = product_analysis
            .iter()
            .map(|p| p["revenue"].as_f64().unwrap_or(0.0))
            .sum();
        let avg_discount: f64 = products
            .iter()
            .map(|p| {
                let base = csv_util::record_get_f64(p, "base_price");
                let current = csv_util::record_get_f64(p, "current_price");
                if base > 0.0 {
                    (1.0 - current / base) * 100.0
                } else {
                    0.0
                }
            })
...
impl Default for PricingOptimizationAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn make_ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response(
            "Widget A is underpriced. Consider raising price 10%.",
        ))
    }
    ...
    async fn test_pricing() {
        ...
        let result = PricingOptimizationAgent::new()
    ...
    async fn test_pricing_status() {
        ...
        let names: Vec<&str> = products
            ...
            .collect();
        let statuses: Vec<&str> = products
            .iter()
            .map(|p| p["pricing_status"].as_str().unwrap())
            .collect();
    ...
    async fn test_empty() {
        let ctx = make_ctx();
        assert!(PricingOptimizationAgent::new()
            .execute_with_context(serde_json::json!({ "products": "" }), &ctx)
            .await
            .is_err());
    }
