...
impl Agent for PipelineHealthAgent {
    fn manifest(&self) -> AgentManifest {
        AgentManifest {
            ...
            description: "Analyze pipeline velocity, stage conversion, and identify bottlenecks".to_string(),
            tier: AgentTier::Free,
    ...
    async fn execute_with_context(&self, input: serde_json::Value, ctx: &dyn AgentContext) -> AppResult<serde_json::Value> {
        ...
        let stages = vec!["discovery", "qualification", "proposal", "negotiation", "won", "lost"];
        ...
        let discovery_to_qual = if discovery > 0 { qualification as f64 / discovery as f64 } else { 0.0 };
        let qual_to_proposal = if qualification > 0 { proposal as f64 / qualification as f64 } else { 0.0 };
        let close_rate = if (won + lost) > 0 { won as f64 / (won + lost) as f64 } else { 0.0 };
...
impl Default for PipelineHealthAgent { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    async fn test_pipeline_health() {
        ...
        let result = PipelineHealthAgent::new().execute_with_context(input, &ctx).await.unwrap();
    ...
    async fn test_empty_deals() {
        let ctx = make_ctx();
        assert!(PipelineHealthAgent::new().execute_with_context(serde_json::json!({ "deals": "" }), &ctx).await.is_err());
    }
