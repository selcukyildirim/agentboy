...
impl Agent for WinLossAnalysisAgent {
    ...
    async fn execute_with_context(
        ...
        let won_value: f64 = won
            .iter()
            .map(|r| csv_util::record_get_f64(r, "value"))
            .sum();
        let lost_value: f64 = lost
            .iter()
            .map(|r| csv_util::record_get_f64(r, "value"))
            .sum();
        ...
        let mut loss_summary: Vec<serde_json::Value> = loss_reasons
            .iter()
            .map(|(reason, count)| serde_json::json!({ "reason": reason, "count": count }))
            .collect();
        loss_summary.sort_by(|a, b| {
            b["count"]
                .as_u64()
                .unwrap_or(0)
                .partial_cmp(&a["count"].as_u64().unwrap_or(0))
...
impl Default for WinLossAnalysisAgent {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn make_ctx() -> MockAgentContext {
        MockAgentContext::new(MockLlmProvider::with_response(
            "Price is the top loss reason. Improve value proposition.",
        ))
    }
    ...
    async fn test_win_loss() {
        ...
        let result = WinLossAnalysisAgent::new()
    ...
    async fn test_empty() {
        let ctx = make_ctx();
        assert!(WinLossAnalysisAgent::new()
