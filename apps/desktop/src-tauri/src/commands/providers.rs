...
fn spec_cache() -> &'static Mutex<Option<CachedSpec>> {
    static C: OnceLock<Mutex<Option<CachedSpec>>> = OnceLock::new();
...
pub async fn active_spec_cached() -> Option<(String, Option<String>, Option<String>, Option<String>)>
{
    ...
    let (provider, base_url, model) = active_provider_meta()?;
    let api_key = read_secret(&provider).await;
    let spec = CachedSpec {
